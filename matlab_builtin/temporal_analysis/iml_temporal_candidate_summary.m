function result = iml_temporal_candidate_summary(varargin)
...
if ~isempty(varargin) && iscell(varargin{1}), frames = varargin{1}; else, frames = iml_get_sequence(); end
        ...
        result = struct('method','iml_temporal_candidate_summary','status','candidate','frame_count',n,'coverage',coverage,'persistence',persistence,'change_rate',change);
        iml_register_feature('iml_temporal_candidate_summary_persistence',persistence,'fraction'); iml_register_candidate_result('iml_temporal_candidate_summary','candidate','Sequence heuristic; timing/profile dependent.');
        iml_add_provenance('iml_temporal_candidate_summary','v11 temporal heuristic'); if n < 2, iml_add_warning('iml_temporal_candidate_summary: sequence has fewer than two frames.'); end
        iml_save_matrix('iml_temporal_candidate_summary_coverage',coverage);
        end
