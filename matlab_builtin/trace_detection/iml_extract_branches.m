function result = iml_extract_branches(varargin)
        % IML_EXTRACT_BRANCHES
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_extract_branches:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_extract_branches', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_extract_branches_coverage', score, 'fraction');
        iml_register_candidate_result('iml_extract_branches', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_extract_branches', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_extract_branches: weak candidate or no echo.'); end
        iml_save_matrix('iml_extract_branches_mask', double(mask));
        end
