function result = iml_trace_global_threshold(varargin)
        % IML_TRACE_GLOBAL_THRESHOLD
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_trace_global_threshold:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_trace_global_threshold', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_trace_global_threshold_coverage', score, 'fraction');
        iml_register_candidate_result('iml_trace_global_threshold', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_trace_global_threshold', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_trace_global_threshold: weak candidate or no echo.'); end
        iml_save_matrix('iml_trace_global_threshold_mask', double(mask));
        end
