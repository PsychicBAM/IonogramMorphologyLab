function result = iml_trace_skeleton(varargin)
        % IML_TRACE_SKELETON
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_trace_skeleton:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_trace_skeleton', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_trace_skeleton_coverage', score, 'fraction');
        iml_register_candidate_result('iml_trace_skeleton', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_trace_skeleton', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_trace_skeleton: weak candidate or no echo.'); end
        iml_save_matrix('iml_trace_skeleton_mask', double(mask));
        end
