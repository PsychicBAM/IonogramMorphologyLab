function result = iml_estimate_background(varargin)
        % IML_ESTIMATE_BACKGROUND
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_estimate_background:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_estimate_background', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_estimate_background_coverage', score, 'fraction');
        iml_register_candidate_result('iml_estimate_background', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_estimate_background', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_estimate_background: weak candidate or no echo.'); end
        iml_save_matrix('iml_estimate_background_mask', double(mask));
        end
