function result = iml_build_candidate_report(varargin)
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_build_candidate_report:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_build_candidate_report', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_build_candidate_report_coverage', score, 'fraction');
        iml_register_candidate_result('iml_build_candidate_report', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_build_candidate_report', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_build_candidate_report: weak candidate or no echo.'); end
        iml_save_matrix('iml_build_candidate_report_mask', double(mask));
        end
