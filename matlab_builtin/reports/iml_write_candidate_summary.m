function result = iml_write_candidate_summary(varargin)
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_write_candidate_summary:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_write_candidate_summary', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_write_candidate_summary_coverage', score, 'fraction');
        iml_register_candidate_result('iml_write_candidate_summary', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_write_candidate_summary', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_write_candidate_summary: weak candidate or no echo.'); end
        iml_save_matrix('iml_write_candidate_summary_mask', double(mask));
        end
