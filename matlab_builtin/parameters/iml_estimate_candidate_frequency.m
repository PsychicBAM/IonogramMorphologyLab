function result = iml_estimate_candidate_frequency(varargin)
        % IML_ESTIMATE_CANDIDATE_FREQUENCY
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_estimate_candidate_frequency:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_estimate_candidate_frequency', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_estimate_candidate_frequency_coverage', score, 'fraction');
        iml_register_candidate_result('iml_estimate_candidate_frequency', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_estimate_candidate_frequency', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_estimate_candidate_frequency: weak candidate or no echo.'); end
        iml_save_matrix('iml_estimate_candidate_frequency_mask', double(mask));
        end
