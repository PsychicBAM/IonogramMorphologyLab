function result = iml_segment_es_trace(varargin)
        % IML_SEGMENT_ES_TRACE
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_segment_es_trace:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_segment_es_trace', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_segment_es_trace_coverage', score, 'fraction');
        iml_register_candidate_result('iml_segment_es_trace', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_segment_es_trace', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_segment_es_trace: weak candidate or no echo.'); end
        iml_save_matrix('iml_segment_es_trace_mask', double(mask));
        end
