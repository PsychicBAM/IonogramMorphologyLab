function result = iml_separate_f1_f2_candidate(varargin)
        % IML_SEPARATE_F1_F2_CANDIDATE
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_separate_f1_f2_candidate:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_separate_f1_f2_candidate', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_separate_f1_f2_candidate_coverage', score, 'fraction');
        iml_register_candidate_result('iml_separate_f1_f2_candidate', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_separate_f1_f2_candidate', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_separate_f1_f2_candidate: weak candidate or no echo.'); end
        iml_save_matrix('iml_separate_f1_f2_candidate_mask', double(mask));
        end
