function result = iml_estimate_hF2_prime_candidate(varargin)
        % IML_ESTIMATE_HF2_PRIME_CANDIDATE
...
if ~isempty(varargin) && isnumeric(varargin{1}), frame = varargin{1}; else, frame = iml_get_current_frame(); end
        if ~isnumeric(frame) || ~ismatrix(frame) || isempty(frame), error('iml_estimate_hF2_prime_candidate:invalidFrame', 'Expected a numeric 2-D frame.'); end
        ...
        result = struct('method', 'iml_estimate_hF2_prime_candidate', 'status', 'candidate', 'value', value, ...
          'frequency_candidate', freq(column), 'range_candidate', range(row), ...
          'column', column, 'row', row, 'band_fraction', band_fraction);
        iml_register_feature('iml_estimate_hF2_prime_candidate_value', value, 'normalized');
        iml_register_candidate_result('iml_estimate_hF2_prime_candidate', 'candidate', 'Profile-dependent heuristic measurement.');
        iml_add_provenance('iml_estimate_hF2_prime_candidate', 'v11 base MATLAB measurement');
        if value < 0.2, iml_add_warning('iml_estimate_hF2_prime_candidate: low-confidence measurement.'); end
        end
