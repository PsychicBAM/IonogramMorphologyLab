function result = iml_count_trace_branches(varargin)
        % IML_COUNT_TRACE_BRANCHES
...
if ~isempty(varargin) && isnumeric(varargin{1}), frame = varargin{1}; else, frame = iml_get_current_frame(); end
        if ~isnumeric(frame) || ~ismatrix(frame) || isempty(frame), error('iml_count_trace_branches:invalidFrame', 'Expected a numeric 2-D frame.'); end
        ...
        result = struct('method', 'iml_count_trace_branches', 'status', 'candidate', 'value', value, ...
          'frequency_candidate', freq(column), 'range_candidate', range(row), ...
          'column', column, 'row', row, 'band_fraction', band_fraction);
        iml_register_feature('iml_count_trace_branches_value', value, 'normalized');
        iml_register_candidate_result('iml_count_trace_branches', 'candidate', 'Profile-dependent heuristic measurement.');
        iml_add_provenance('iml_count_trace_branches', 'v11 base MATLAB measurement');
        if value < 0.2, iml_add_warning('iml_count_trace_branches: low-confidence measurement.'); end
        end
