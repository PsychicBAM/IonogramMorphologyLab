function result = iml_measure_branch_separation(varargin)
        % IML_MEASURE_BRANCH_SEPARATION
...
if ~isempty(varargin) && isnumeric(varargin{1}), frame = varargin{1}; else, frame = iml_get_current_frame(); end
        if ~isnumeric(frame) || ~ismatrix(frame) || isempty(frame), error('iml_measure_branch_separation:invalidFrame', 'Expected a numeric 2-D frame.'); end
        ...
        result = struct('method', 'iml_measure_branch_separation', 'status', 'candidate', 'value', value, ...
          'frequency_candidate', freq(column), 'range_candidate', range(row), ...
          'column', column, 'row', row, 'band_fraction', band_fraction);
        iml_register_feature('iml_measure_branch_separation_value', value, 'normalized');
        iml_register_candidate_result('iml_measure_branch_separation', 'candidate', 'Profile-dependent heuristic measurement.');
        iml_add_provenance('iml_measure_branch_separation', 'v11 base MATLAB measurement');
        if value < 0.2, iml_add_warning('iml_measure_branch_separation: low-confidence measurement.'); end
        end
