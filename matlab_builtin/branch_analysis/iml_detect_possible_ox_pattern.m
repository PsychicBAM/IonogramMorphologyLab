function result = iml_detect_possible_ox_pattern(varargin)
...
frame = local_frame(varargin);
        ...
        if ~ok, error('iml_detect_possible_ox_pattern:invalidFrame', '%s', message); end
        ...
        result = struct('method', 'iml_detect_possible_ox_pattern', 'status', 'candidate', ...
          'score', score, 'mask', mask, 'frequency_axis', freq, ...
          'range_axis', range, 'band_fraction', band_fraction);
        iml_register_feature('iml_detect_possible_ox_pattern_coverage', score, 'fraction');
        iml_register_candidate_result('iml_detect_possible_ox_pattern', 'candidate', ...
          'Heuristic profile-dependent candidate; non-causal.');
        iml_add_provenance('iml_detect_possible_ox_pattern', 'v11 built-in heuristic');
        if score < 0.002, iml_add_warning('iml_detect_possible_ox_pattern: weak candidate or no echo.'); end
        iml_save_matrix('iml_detect_possible_ox_pattern_mask', double(mask));
        end
