function passed = iml_test_synthetic_trace()
        % IML_TEST_SYNTHETIC_TRACE
...
rng(11); [c,r]=meshgrid(1:80,1:60); frame=0.02*rand(size(c))+exp(-((r-20-0.2*c).^2)/8);
        ...
        if ~passed, error('iml_test_synthetic_trace:failed','Synthetic-only candidate test failed.'); end
        iml_add_provenance('iml_test_synthetic_trace','Synthetic-only test; not scientific validation.');
        end
